...
export default function ClientMessagesPage() {
  ...
  const [adminName, setAdminName] = useState("Ideolyte");
  ...
  async function handleSend(e: React.FormEvent) {
    ...
    formData.set("receiver_id", adminId);
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Messages" description="Chat with your project team" />
      <Card className="flex flex-col h-[calc(100vh-240px)] min-h-[400px]">
        <div className="flex items-center gap-3 p-4 border-b border-gray-100">
          <Avatar name={adminName} size="sm" />
          <div>
            <p className="text-sm font-medium text-gray-900">{adminName}</p>
            <p className="text-xs text-gray-500">Ideolyte · Project Lead</p>
          </div>
        </div>
        <CardContent className="flex-1 overflow-y-auto p-4 space-y-3">
          {messages.length === 0 ? (
            <EmptyState title="No messages yet" description="Send a message to start the conversation" icon={<MessageSquare className="h-8 w-8" />} />
          ) : (
            messages.map((msg) => {
              const isMe = msg.sender_id === user?.id;
              return (
                <div key={msg.id} className={`flex gap-3 ${isMe ? "justify-end" : ""}`}>
                  {!isMe && <Avatar name={msg.sender?.full_name || adminName} size="sm" />}
        ...
        <form onSubmit={handleSend} className="border-t border-gray-100 p-4">
          <div className="flex items-center gap-2">
            <Input placeholder="Type a message..." value={msgInput} onChange={e => setMsgInput(e.target.value)} className="flex-1" disabled={sending || !adminId} />
            <Button size="icon" type="submit" disabled={sending || !msgInput.trim() || !adminId}><Send className="h-4 w-4" /></Button>
          </div>
        </form>
      </Card>
    </div>
